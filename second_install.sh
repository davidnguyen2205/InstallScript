...
# Optimize Odoo performance with enable multiprocessing mode for 2 cores CPU, 4GB RAM
sudo su root -c "printf 'workers = 5\n' >> /etc/${OE_CONFIG}.conf"
sudo su root -c "printf 'limit_memory_hard = 3840\n' >> /etc/${OE_CONFIG}.conf"
sudo su root -c "printf 'limit_memory_soft = 3200\n' >> /etc/${OE_CONFIG}.conf"
